...
{
    public class GetDegreeQuery : LookupInputModel, IRequest<LookupOutputModel>
    {
    }

    public class GetDegreeQueryHandler : IRequestHandler<GetDegreeQuery, LookupOutputModel>
    {
        private readonly IApplicationDbContext _context;
        private readonly IMapper _mapper;

        public GetDegreeQueryHandler(IApplicationDbContext context, IMapper mapper)
        {
            _context = context;
            _mapper = mapper;
        }

        public async Task<LookupOutputModel> Handle(GetDegreeQuery request, CancellationToken cancellationToken)
        {
            ...
            IEnumerable<Degree> queryOrdered = query;

            if (request.OrderColumn != null && request.OrderColumn.OrderByColumn != null)
            {
                queryOrdered = LinqHelper.OrderByPropertyName<Degree>(query, request.OrderColumn.OrderByColumn, request.OrderColumn.OrderByAsc);
            }
            ...
            try
            {
                var retVal = _mapper.Map(sourceList, typeof(List<Degree>), typeof(List<LookupSingleResultModel>));
