...
import { PrepopulateProjectInfo } from '../../services/ProjectServices';
...
export const PrepopulateSearchProject = ({ author, match, baseUrl, noProjectsDisplay, projectsMapFunction }: any) => 
{
    const PER_PAGE = 5;
    ...
    const [prepopulateFilter, setPrepopulateFilter] = useState<any>({});
    const [firstPrepopulate, setFirstPrepopulate] = useState(false);

    const [appliedFilterSearch, setAppliedFilterSearch] = useState(false);

    const currentPageData = projects?.map(projectsMapFunction);

    useEffect(()=> 
    { 
        setExpertiseSelectedValue(author.expertiseAreas?.map((area: {id: number, value: string}) => ({"name": area.value, "value": area.id})))
        setKindOfWorkSelectedValue(author.kindOfWorks?.map((area: {id: number, value: string}) => ({"name": area.value, "value": area.id})))
        setLanguageSelectedValue(author.languages?.map((area: { id: number, value: string }) => ({"name": area.value, "value": area.id})))
        setDegreeSelectedValue({ "name": author.highestDegree.value, "value": author.highestDegree.id});
        
        setPrepopulateFilter({ 
            areaOfExpertise: author.expertiseAreas.map((v: any) => v.id),
            kindOfWork: author.kindOfWorks.map((v: any) => v.id),
            language: author.languages.map((v: any) => v.id),
            highestDegree: { 'name': author.highestDegree.value, 'value': author.highestDegree.id },
            pagesFrom: 0,
            pagesTo: 10000,
            deadline: '',
            currentPage: 0
    ...
    useEffect(() => 
    {
        ...
        if(firstPrepopulate === false && JSON.stringify(prepopulateFilter) !== '{}' && prepopulateFilter.areaOfExpertise.map((v: any) => v)[0] !== 0)
        {
            setLoading(true);
            PrepopulateProjectInfo(prepopulateFilter, baseUrl, alert).then((data) => {
                setProjects(data?.items ?? []);
                setNoOfProjects(data?.totalCount ?? 0);
                setLoading(false);
                setFirstTimeLookup(false);
            })
            setFirstPrepopulate(true);
        }
        else if(firstPrepopulate === true && appliedFilterSearch === true)
        {
            setLoading(true);
            PrepopulateProjectInfo(appliedFilters, baseUrl, alert).then((data) => {
                setProjects(data?.items ?? []);
                setNoOfProjects(data?.totalCount ?? 0);
                setLoading(false);
                setFirstTimeLookup(false);
    ...
    }, [appliedFilters, prepopulateFilter])
    ...
    return (
        <Col sm="12">
            <Formik initialValues={{ deadline: '' }}
                onSubmit={(values) => {
                    setCurrentPage(0);
                    setAppliedFilterSearch(true);
                    setAppliedFilters({
                        areaOfExpertise: selectedExpertiseValue.map((v: any) => v.value ),
                        kindOfWork: selectedKindOfWorkValue.map((v: any) => v.value ),
                        language: selectedLanguageValue.map((v: any) => v.value),
...
export default PrepopulateSearchProject;